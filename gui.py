...
def table(main, browse):
    ...
    if itr > 0:
        ...
        Button(bottom, text="Get Back", padx=40, command=lambda: back(tble, main)).grid(row=0, column=0)
        Button(bottom, text="Save As", padx=40, command=lambda: savelocation(valuedict)).grid(row=0, column=1)
        Button(bottom, text="Calculate Remark", padx=20, command=lambda: remarks(tble, list(nandk.values()))).grid(row=0, column=2)
